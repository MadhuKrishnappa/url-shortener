...
func CreateShortUrl(c *gin.Context) {
	...
	if IsShortUrlCreated(shortUrl, creationRequest.LongUrl, c, host) {
		return
	}

	if IsShortURLCountLimitReached(c, shortUrl) {
		return
	}

	shortUrlMapping := db.SaveUrlMapping(shortUrl, creationRequest.LongUrl)
...
func IsShortUrlCreated(shortUrl string, longUrl string, c *gin.Context, host string) bool {
	responseUrlMapping := db.RetrieveInitialUrl(shortUrl)
